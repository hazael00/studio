...
import { MapPin, ExternalLink, Download, Settings, BarChartHorizontalBig } from 'lucide-react';
  ...
  return (
    <Card className="flex flex-col overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1">
      ...
      <CardFooter className="flex flex-col sm:flex-row gap-2">
        ...
        {track.setupGuidePdfUrl && (
            <Button variant="secondary" size="sm" className="flex-1">
                <Download className="mr-2 h-4 w-4" /> Setup
            </Button>
        )}
        {track.strategyGuidePdfUrl && (
            <Button variant="secondary" size="sm" className="flex-1">
                <BarChartHorizontalBig className="mr-2 h-4 w-4" /> Estrategia
            </Button>
        )}
      </CardFooter>
    </Card>
